...
def main():
    ...
    fname = "trading_card_shop_PL_v2.xlsx"
    wb.save(fname)
    print(f"✅ 保存完了: {fname}")
    print()
    print("📋 シート構成:")
    ...
    print("  4. ダッシュボード  ← 月次推移テーブル・収益性指標")
    print()
    print("📌 目標数値（月次）:")
    total_sales = TARGET["new_sales"] + TARGET["used_sales"]
    new_gross = TARGET["new_sales"] * (1 - TARGET["new_cost_rate"])
    used_gross = TARGET["used_sales"] * (1 - TARGET["used_cost_rate"])
    gross = new_gross + used_gross
    sga_total = sum(SGA_DETAIL.values())
    op = gross - sga_total
    print(f"  売上合計:   {total_sales:>12,} 円")
    print(f"  粗利:       {gross:>12,.0f} 円  (粗利率 {gross/total_sales*100:.1f}%)")
    print(f"  販管費:     {sga_total:>12,} 円")
    print(f"  営業利益:   {op:>12,.0f} 円  (利益率 {op/total_sales*100:.1f}%)")
