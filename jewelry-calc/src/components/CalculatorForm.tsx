import { METAL_GRADE_OPTIONS, METAL_PRICES } from '../constants';
import type { FormData, MarketRate } from '../types';
import { TaxTabs } from './TaxTabs';
import { parseNum, getTaxDivisor } from '../calc';
...
export function CalculatorForm({ form, onChange, activeMarketRate, effectiveMetalPrice }: Props) {
  ...
  const masterPrice = METAL_PRICES[form.metalGrade] ?? 0;
    ...
    <div className="card space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="品位 ★">
          <select
            className="input"
            value={form.metalGrade}
            onChange={set('metalGrade')}
          >
              ...
              <span>
                ...
                {activeMarketRate && (
                  <span className="ml-1 text-xs text-gray-400">
                    (基準: {masterPrice.toLocaleString()} × {activeMarketRate.rate})
                  </span>
                )}
              </span>
            ) : '—'}
      ...
      <Field label="中石重量" unit="ct">
        <input
          ...
          step="0.01"
          placeholder="0.00"
          value={form.centerStoneWeight}
          onChange={set('centerStoneWeight')}
        />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="脇石重量" unit="ct">
          <input
            ...
            step="0.001"
            placeholder="0.000"
            value={form.sideStoneWeight}
            onChange={set('sideStoneWeight')}
          />
        </Field>
        <Field label="脇石単価" unit="円/ct">
          <input
            ...
            min="0"
            placeholder="0"
            value={form.sideStonePricePerCt}
            onChange={set('sideStonePricePerCt')}
          />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="MD単価" unit="円/ct">
          <input
            ...
            min="0"
            placeholder="0"
            value={form.mdPricePerCt}
            onChange={set('mdPricePerCt')}
          />
        </Field>
      </div>
    </div>
